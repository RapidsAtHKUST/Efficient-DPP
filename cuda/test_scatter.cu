...
__global__
void scatter_kernel(int *d_in, int *d_out, int *d_idx, int num) {
...
void test_scatter(int len, CUDATimeStat *timing) {
...
#pragma omp parallel for
    ...
    for(auto p = 1; p <= 32; p<<= 1) {
        for(int i = 0; i < EXPERIMENT_TIMES; i++) {
            ...
            execKernel(scatter_kernel, grid_size, block_size, timing, false, d_in, d_out, d_idx, len);
...
int main(int argc, char *argv[]) {
    ...
    CUDATimeStat timing;
    test_scatter(stoi(argv[1]), &timing);
    return 0;
}
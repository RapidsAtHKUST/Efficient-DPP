...
template<class T> bool testGather( 
	...
	for(int i = 0; i < r_len; i++) {
		if (
#ifdef RECORDS
		h_dest_keys[i] != h_source_keys[loc[i]] ||
#endif
		h_dest_values[i] != h_source_values[loc[i]]) 
		{
			res = false;
			break;
		}
...
template bool testGather<float>( 
#ifdef RECORDS
	int *source_keys, 
#endif
...
template bool testGather<double>( 
#ifdef RECORDS
	int *source_keys, 
#endif
...
template bool testGather_mul<int>( 
#ifdef RECORDS
	int *source_keys, 
#endif
	int *source_values, int r_len, int* loc,
...
template bool testGather_mul<long>( 
#ifdef RECORDS
	int *source_keys, 
#endif
	long *source_values, int r_len, int* loc,
...
template bool testGather_mul<float>( 
#ifdef RECORDS
	int *source_keys, 
#endif
	float *source_values, int r_len, int* loc,
...
template bool testGather_mul<double>( 
#ifdef RECORDS
	int *source_keys, 
#endif
	double *source_values, int r_len, int* loc,

...
double scan(cl_mem &d_source, int length, int isExclusive, PlatInfo info, int localSize)
{
    ...
    int element_per_thread = ELEMENT_PER_THREAD;
    ...
    if (firstLevelBlockNum == 1) {      //length <= element_per_block, only 1 level is enough
        ...
        status |= clSetKernelArg(scanBlockKernel, argsNum++, sizeof(int), &warpSize);
        status |= clSetKernelArg(scanBlockKernel, argsNum++, sizeof(int), &element_per_thread);
        ...
        status |= clSetKernelArg(scanBlockKernel, argsNum++, sizeof(int) * WARPSIZE, NULL);       
        ...
        status |= clSetKernelArg(scanAddBlockKernel, argsNum++, sizeof(int), &element_per_thread);     
        checkErr(status, ERR_SET_ARGUMENTS);
